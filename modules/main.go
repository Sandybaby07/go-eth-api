...
// GetRecentBlock from blockchain
func GetRecentBlock(client ethclient.Client, limit string) string {
	// We add a recover function from panics to prevent our API from crashing due to an unexpected error
	defer func() {
		if err := recover(); err != nil {
			fmt.Println(err)
...
// GetBlockByNumber from blockchain
func GetBlockByNumber(client ethclient.Client, blockNum string) *Models.Block {
	// We add a recover function from panics to prevent our API from crashing due to an unexpected error
	defer func() {
		if err := recover(); err != nil {
			fmt.Println(err)
	...
	// Query the latest block
	b, _ := strconv.ParseInt(blockNum, 10, 64)
...
func GetTxByHash(client ethclient.Client, hash common.Hash) *Models.Transaction {
	...
	res := db.TxHash(tx.Hash().String(), tx.Value().String(), strconv.FormatUint(tx.Gas(), 10), strconv.FormatUint(tx.GasPrice().Uint64(), 10), strconv.FormatUint(tx.Nonce(), 10), "0x0000000000", tx.To().String(), pending)
